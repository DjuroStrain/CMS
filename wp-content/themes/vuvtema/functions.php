...
//add_action( 'admin_enqueue_scripts', 'UcitajCssTeme' );

function UcitajCssSelect()
{		
	wp_enqueue_style('select_css', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css');
}

add_action( 'admin_enqueue_scripts', 'UcitajCssSelect' );
...
function registriraj_predmet_cpt() {
	...
	$labels = array(
		...
		'search_items'          => __( 'Traži predmet', 'vuv' ),
		...
		'remove_featured_image' => __( 'Izbriši glavnu lsiku', 'vuv' ),
	...
	$args = array(
		...
		'description'           => __( 'Post Type Predmet', 'vuv' ),
		'labels'                => $labels,
...
function registriraj_taksonomiju_semestar() {
	...
	$labels = array(
		...
		'update_item'                => __( 'Ažuriraj semestart', 'vuv' ),
		...
		'search_items'               => __( 'Pretraga', 'vuv' ),
		'not_found'                  => __( 'NIje pronađeno', 'vuv' ),
		'no_terms'                   => __( 'Nema semestra', 'vuv' ),
		'items_list'                 => __( 'Lista semestra', 'vuv' ),
		'items_list_navigation'      => __( 'Navigacija', 'vuv' ),
	);
	$args = array(
		'labels'                     => $labels,
...
function daj_predmete( $slug, $slug2 )
{
	$args = array(
		'posts_per_page' => -1,
		'post_type' => 'predmet',
	...
	$sHtml .= '<li class="text-center"><a href="'.$sPredmetUrl.'">'.$sPredmetNaziv.'</a></li>';
	}
	$sHtml .= "</ul>";
	return $sHtml;
}
...
{
	add_meta_box( 'vuv_predmet_nastavnici', 'Nastavnici predmeta', 'html_meta_box_nastavnici', 'predmet');
}
...
function daj_semestre( $slug )
{
	...
	foreach ($nastavnici as $nastavnik)
	{
		...
		$satiPredavanja = get_post_meta($nastavnik->ID,'predmet_sati_predavanja',true);
		$satiLV = get_post_meta($nastavnik->ID,'predmet_sati_lv_vjezbi',true);
		$satiKV = get_post_meta($nastavnik->ID,'predmet_sati_kv',true);
		$sNastavnikUrl = $nastavnik->guid;
		$sNastavnikNaziv = $nastavnik->post_title;
		$sHtml .= '<td><a href="'.$sNastavnikUrl.'">'.$sNastavnikNaziv.'</a></td>';
		$sHtml .= '<td>'.$ects.'</td>';
		$sHtml .= '<td>'.$satiPredavanja.'</td>';
		$sHtml .= '<td>'.$satiLV.'</td>';
		$sHtml .= '<td>'.$satiKV.'</td></tr>';
	}
	$sHtml .= "</tbody></table>";
	return $sHtml;
}
