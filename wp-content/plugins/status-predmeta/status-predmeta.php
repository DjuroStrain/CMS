...
{
    add_meta_box('vsmti_predmet_status', 'Status predmeta', 'html_meta_box_status_predmeta', 'predmet');
}
...
function html_meta_box_status_predmeta($post)
{
    ...
    <option value="obavezan" ' . (($status_predmeta == 'obavezan' || $status_predmeta == '') ? 'selected' : '') . '>Obavezan</option>
    <option value="izborni" ' . (($status_predmeta == 'izborni') ? 'selected' : '') . '>Izborni</option>
    </select>
    </div>';
}
...
function spremi_status_predmeta($post_id)
{
    ...
    $is_valid_nonce_status_predmeta = (isset($_POST['status_predmeta_nonce']) && wp_verify_nonce($_POST['status_predmeta_nonce'], basename(__FILE__))) ? 'true' : 'false';
